...
{
    /// <summary>
    /// Editor script tự động thiết lập Phase 19A: Knight Animation Setup (làm giống Archer).
    /// Chạy qua thanh menu: Dev5 / Setup Phase 19A - Knight Animation
    /// </summary>
    public static class Dev5Phase19AKnightAnimationSetupEditor
    {
        ...
        public static void SetupPhase19AKnightAnimation()
        {
            ...
            {
                Debug.LogWarning($"[Phase19AKnightSetup] Đang chạy trên scene '{activeScene.name}' thay vì '{SCENE_REQUIRED}'.");
            }

            Undo.SetCurrentGroupName("Phase 19A Knight Animation Setup");
            int undoGroup = Undo.GetCurrentGroup();

            // 1. Quét và phân loại các file FBX từ thư mục Animation/
            string idleFbx = null;
            string moveFbx = null;
            ...
            {
                string[] fbxFiles = System.IO.Directory.GetFiles(ANIMATIONS_DIR, "*.fbx", System.IO.SearchOption.AllDirectories);
                foreach (string file in fbxFiles)
                {
                    string normalizedPath = file.Replace('\\', '/');
                    string fileNameLower = System.IO.Path.GetFileName(file).ToLower();

                    if (fileNameLower.Contains("boom_dance"))
                    {
                        idleFbx = normalizedPath;
                    }
                    else if (fileNameLower.Contains("you_groove"))
                    {
                        moveFbx = normalizedPath;
                    }
                    else if (fileNameLower.Contains("attack"))
                    {
                        attackFbx = normalizedPath;
                    }
                    else if (fileNameLower.Contains("skill_01"))
                    {
            ...
            if (string.IsNullOrEmpty(idleFbx))
            {
                Debug.LogError("[Phase19AKnightSetup] Không tìm thấy file FBX chứa animation Boom Dance làm model gốc!");
                Undo.CollapseUndoOperations(undoGroup);
                return;
            }

// 2. Cấu hình Rig = Humanoid (CreateFromThisModel) và Loop settings cho từng FBX animation
            // Lưu ý: mỗi FBX đều chứa skeleton đầy đủ nên dùng CreateFromThisModel giống Archer
            ...
            // 4. Lấy các clip hoạt ảnh thực từ FBX
            AnimationClip idleClip = GetClipFromFBX(idleFbx);
            AnimationClip moveClip = GetClipFromFBX(moveFbx);
            AnimationClip attackClip = GetClipFromFBX(attackFbx);
            AnimationClip deathClip = GetClipFromFBX(deathFbx);

            // Báo cáo kết quả map hoạt ảnh
            Debug.Log($"[Phase19AKnightSetup] Mapping clips:\n" +
                      $"- Idle: {(idleFbx != null ? System.IO.Path.GetFileName(idleFbx) : "null")}\n" +
                      $"- Move: {(moveFbx != null ? System.IO.Path.GetFileName(moveFbx) : "null")}\n" +
                      $"- Attack: {(attackFbx != null ? System.IO.Path.GetFileName(attackFbx) : "null")}\n" +
                      $"- Death: {(deathFbx != null ? System.IO.Path.GetFileName(deathFbx) : "null")}");

            // 5. Tải Animator Controller
            ...
            if (controller == null)
            {
                ...
                Debug.Log($"[Phase19AKnightSetup] Đã tạo Animator Controller mới tại: {CONTROLLER_PATH}");
            }
            ...
            // 8. Cập nhật Prefab Knight sử dụng Model từ Boom Dance FBX (có đủ xương & skin) làm visual chính
            RestructureKnightPrefab(KNIGHT_PREFAB_PATH, controller, idleFbx);
            ...
            Debug.Log($"[Phase19AKnightSetup] ✅ Setup Phase 19A - Knight Animation hoàn tất!");
        }
        ...
        private static void ConfigureModelImporter(string fbxPath, bool isLoop)
        {
            ...
            if (modelImporter != null)
            {
                ...
                // Dùng CreateFromThisModel cho tất cả FBX (mỗi FBX Meshy đều chứa skeleton)
                // Giống hệt Archer: không dùng CopyFromOther vì gây lỗi 'No human bone found'
                ...
                if (changed)
                {
                    ...
                    Debug.Log($"[Phase19AKnightSetup] Cấu hình Rig=Humanoid (CreateFromThisModel) và Loop={isLoop} cho: {fbxPath}");
                }
            }
        }
        ...
        private static void RestructureKnightPrefab(string path, AnimatorController controller, string modelFbxPath)
        {
            GameObject prefabAsset = AssetDatabase.LoadAssetAtPath<GameObject>(path);
            if (prefabAsset == null)
            {
                Debug.LogWarning($"[Phase19AKnightSetup] Không tìm thấy prefab Knight tại: {path}");
                return;
            }
            ...
            if (instance == null)
            {
                Debug.LogError($"[Phase19AKnightSetup] Không thể instantiate prefab Knight: {prefabAsset.name}");
                return;
            }
            ...
            if (animCtrl == null)
            {
                ...
                Debug.Log($"[Phase19AKnightSetup] Đã thêm CharacterAnimationController vào root của Knight prefab.");
            }
            ...
            string deathClipPath = "Assets/Project/Dev5_Art/Audio/Tank/Voice/voice_tank/Tank_Death.wav";
            ...
            // Gán vào CharacterAnimationController
            ...
            Debug.Log($"[Phase19AKnightSetup] Đã tự động cấu hình AudioSource và gán AudioClips cho Knight Prefab.");
                ...
                if (modelSlotChild != null)
                {
                    ...
                    if (modelFbx != null)
                    {
                        ...
                        if (modelInstance != null)
                        {
                            ...
                            // Dịch chuyển model xuống -1 để chân chạm đất giống Archer
                            // (Root prefab đặt ở cellTop + 1.0, CapsuleCollider cao 2 đơn vị, nên chân = localY = -1.0)
                            modelInstance.transform.localPosition = new Vector3(0f, -1f, 0f);
                            modelInstance.transform.localRotation = Quaternion.identity;
                            modelInstance.transform.localScale = Vector3.one;

                            // Cấu hình Material và Texture cho toàn bộ renderers
                            string matDir = "Assets/Project/Dev5_Art/Models/Ironbound_Warlord";
                            string texPath     = matDir + "/Meshy_AI_Ironbound_Warlord_0701110034_texture.png";
                            string metallicPath = matDir + "/Meshy_AI_Ironbound_Warlord_0701110034_texture_metallic.png";
                            string roughnessPath = matDir + "/Meshy_AI_Ironbound_Warlord_0701110034_texture_roughness.png";
                            ...
                            Debug.Log($"[Phase19AKnightSetup] Textures - Base: {(baseTex != null ? "OK" : "NOT FOUND")} | Metallic: {(metallicTex != null ? "OK" : "NOT FOUND")} | Roughness: {(roughnessTex != null ? "OK" : "NOT FOUND")}");

                            // Dùng M_Unit_Knight.mat (đúng path thực tế, không phải M_Warlord.mat)
                            string matPath = "Assets/Project/Dev5_Art/Materials/M_Unit_Knight.mat";
                            ...
                            if (mat != null)
                            {
                                ...
                                // Ràemap material tích hợp trong FBX sang M_Unit_Knight
                                // (thay vì chỉ gán runtime, gày lội bị override bởi embedded material)
                                ModelImporter modelImp = AssetImporter.GetAtPath(modelFbxPath) as ModelImporter;
                                if (modelImp != null)
                                {
                                    var remaps = modelImp.GetExternalObjectMap();
                                    var matKey = new AssetImporter.SourceAssetIdentifier(typeof(Material), "Material_1");
                                    if (!remaps.ContainsKey(matKey) || remaps[matKey] != mat)
                                    {
                                        modelImp.AddRemap(matKey, mat);
                                        modelImp.SaveAndReimport();
                                        Debug.Log($"[Phase19AKnightSetup] Remapped FBX material 'Material_1' -> '{mat.name}' ");
                                    }
                                }
                                ...
                                Debug.Log($"[Phase19AKnightSetup] Đã gán Material '{mat.name}' (URP/Lit) cho {renderers.Length} renderers.");
                            }
                            else
                            {
                                Debug.LogWarning($"[Phase19AKnightSetup] ⚠️ Không tìm thấy M_Unit_Knight.mat tại: {matPath}!");
                            }
                            ...
                            if (foundAvatar != null)
                            {
                                ...
                                Debug.Log($"[Phase19AKnightSetup] ✅ Gán Avatar '{foundAvatar.name}' cho Animator (isValid={foundAvatar.isValid}, isHuman={foundAvatar.isHuman})");
                            }
                            else
                            {
                                Debug.LogWarning($"[Phase19AKnightSetup] ⚠️ Không tìm thấy Avatar trong {modelFbxPath}.");
                            }
                        }
                    }
                }
            ...
            Debug.Log($"[Phase19AKnightSetup] Đã cập nhật thành công Knight Prefab tại: {path}");
        }
    }
}
#endif
